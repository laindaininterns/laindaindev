...
import { fetchAdminProductsCatalog } from '../../services/api';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={loadProducts} className="text-xs text-[#85A6A3] hover:underline font-medium mr-2">
            🔄 Refresh Catalog
          </button>
          <input
            type="text"
          ...
          >
            ...
            <option value="Footwear & Goods">Footwear & Goods</option>
              ...
              <tbody className="divide-y divide-[#E9E8E2]">
                {filtered.map((p) => (
                  <tr key={p.id} className="text-[14px] hover:bg-[#F9F9F6]/50 transition-colors">
                    ...
                    <td className="py-4 text-[#5B5B58]">{p.cat || 'General Wholesale'}</td>
                    <td className="py-4 text-right font-medium">Rs. {p.price ? p.price.toLocaleString() : 0}</td>
                    <td className="py-4 text-right text-[#5B5B58]">{p.moq || 10} units</td>
                    <td className="py-4 text-right font-medium text-black">{(p.stock || 0).toLocaleString()}</td>
