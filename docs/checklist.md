...
## 📧 Stage 3.5: Automated Email System & Verification Tracking (Resend SDK)
- [x] Install `resend` package in Node.js backend
- [x] Create database migration `03_email_verification.sql` adding `is_email_verified` and `email_verification_token` to `users` table
- [x] Build centralized email service (`src/services/emailService.js`) with `sendEmail`, `sendVerificationCode`, `sendSellerStatusAlert`, `sendGuestReceipt`
- [x] Update `register` controller to generate 6-digit numeric OTP code and dispatch `sendVerificationCode`
- [x] Create `POST /api/auth/verify-email` endpoint to validate 6-digit OTP code and flip `is_email_verified` to `true`
- [x] Add post-execution notification hook on `PATCH /api/admin/sellers/:id/status` dispatching `sendSellerStatusAlert`
- [x] Pre-configure local backend environment with `RESEND_API_KEY` and `FROM_EMAIL` placeholders
- [x] Verify clean server boot cycle and clean imports

## 🖥️ Stage 3.6: Admin Dashboard UI & Live Approvals API Integration
- [x] Extracted HTML templates from `sharaf/ui-designs` into `client/admin-designs/` on `mohsin/backend-setup` branch
- [x] Converted `summary.html`, `approvals.html`, `sellers.html`, `buyers.html`, and `products.html` into modular React components under `client/src/components/admin/`
- [x] Implemented role-based login redirect logic in `App.jsx` (`user.role === 'ADMIN'` redirects immediately to Admin Dashboard)
- [x] Wired live backend API call `GET /api/admin/sellers/pending` in `AdminApprovalsPage.jsx`
- [x] Connected Approve/Deny actions to `PATCH /api/admin/sellers/:id/status` triggering live Supabase DB updates & Resend emails
- [x] Verified zero-error production build via Vite (`npm run build`)
...
- [x] Install `resend` package and build email notification service module
